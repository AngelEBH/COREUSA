...
namespace adminfiles
{
    public class FileUploader
    {
        ...
        private bool upload_file(HttpPostedFile file, string destination)
        {
            ...
            if (File.Exists(destination))
                saveTempFileSolicitud(Path.GetFileName(destination), Path.GetExtension(destination), Path.GetDirectoryName(destination));

            return File.Exists(destination);
        }

        // guardar documento en la lista de documentos de la solicitud
        public void saveTempFileSolicitud(string FileName, string typeFile, string destination)
        {
            List<SolicitudesDocumentosViewModel> sessionTempFileName = new List<SolicitudesDocumentosViewModel>();
            var list = (List<SolicitudesDocumentosViewModel>)HttpContext.Current.Session["ListaSolicitudesDocumentos"];
            int tipoDocumento = (int)HttpContext.Current.Session["tipoDoc"];

            if (list == null)
            {
                sessionTempFileName.Add(new SolicitudesDocumentosViewModel()
                {
                    NombreAntiguo = FileName, //nombre con extension
                    fcTipoArchivo = typeFile, //solo extension (.jpg)
                    fcRutaArchivo = destination,//ruta del archivo
                    fiTipoDocumento = tipoDocumento //tipo de documento (cedula, comprobante de ingresos, ect)
                });
                HttpContext.Current.Session["ListaSolicitudesDocumentos"] = sessionTempFileName;
            }
            else
            {
                list.Add(new SolicitudesDocumentosViewModel()
                {
                    NombreAntiguo = FileName, //nombre con extension
                    fcTipoArchivo = typeFile, //solo extension (.jpg)
                    fcRutaArchivo = destination,//ruta del archivo
                ...
                HttpContext.Current.Session["ListaSolicitudesDocumentos"] = list;
            }
        }

        // remover documento de la lista de documentos de la solicitud
        public void removeTempFileSolicitud(string FileName, string typeFile, string destination)
        {
            int tipoDocumento = (int)HttpContext.Current.Session["tipoDoc"];
            var list = (List<SolicitudesDocumentosViewModel>)HttpContext.Current.Session["ListaSolicitudesDocumentos"];
            if (list != null)
            {
                SolicitudesDocumentosViewModel sessionTempFileName = new SolicitudesDocumentosViewModel()
                {
                    NombreAntiguo = FileName, //nombre con extension
                    fcTipoArchivo = typeFile, //solo extension (.jpg)
                    fcRutaArchivo = destination,//ruta del archivo
                    fiTipoDocumento = tipoDocumento
                };
                list.Remove(sessionTempFileName);
                HttpContext.Current.Session["ListaSolicitudesDocumentos"] = list;
            }
        }
        ...
        private string codeToMessage(string code, dynamic item = null)
        {
            ...
            switch (code)
            {
                case "maxRequestLength":
                    message = "El archivo cargado supera el upload_max_filesize especificado en el archivo de configuracion";
                    break;
                case "accepted_file_types":
                    message = "Tipo de archivo no permitido para " + item["old_name"];
                    break;
                case "file_uploads":
                    message = "La opción de subir archivos está desactivada";
                    break;
                case "max_file_size":
                    message = item["old_name"] + " es muy largo";
                    break;
                case "max_files_size":
                    message = "Los archivos son muy pesados";
                    break;
                case "max_number_of_files":
                    message = "Cantidad máxima de archivos excedida";
                    break;
                case "required_and_no_file":
                    message = "No se seleccionó ningún archivo, seleccione uno";
                    break;
                case "invalid_folder_path":
                    message = "La carpeta de destino no existe o no se tienen los permisos de escritura";
                    break;
                default:
