    ...
    var btnFinish = $('<button type="button" id="btnGuardarSolicitud"></button>').text('Finalizar')
        ...
        .on('click', function () {
            ...
            if (modelStateFormPersonal == true && modelStateFormDomiciliar == true && modelStateFormLaboral == true && modelStateFormConyugal == true) {
                ...
                $.ajax({
                    type: "POST",
                    url: 'SolicitudesCredito_RegistrarAval.aspx/RegistrarAval',
...
function LlenarListasPrecalificado() {
    $.ajax({
        type: "POST",
        url: "SolicitudesCredito_RegistrarAval.aspx/LlenarListasPrecalificado",
        contentType: 'application/json; charset=utf-8',
        error: function (xhr, ajaxOptions, thrownError) {
        ...
        success: function (data) {
            ...
            console.log(data.d);
            var ciudades = data.d.ciudades;
            var ddlCiudadResidencia = $("#ddlCiudadResidencia");
            for (var i = 0; i < ciudades.length; i++) {
                ddlCiudadResidencia.append('<option value="' + ciudades[i] + '">' + ciudades[i] + '</option>');
            }
            var productos = data.d.productos;
            var ddlProducto = $("#ddlProducto");
            for (var i = 0; i < productos.length; i++) {
                ddlProducto.append('<option value="' + productos[i].fiIDProducto + '">' + productos[i].fcDescProducto + '</option>');
            }
            var ddlOrigenIngreso = $("#ddlOrigenIngreso");
            ddlOrigenIngreso.append('<option value="Asalariado">Asalariado</option>');
            ddlOrigenIngreso.append('<option value="Comerciante">Comerciante</option>');
        }
    });
}

$("#btnConsultarCliente2").click(function () {


    if ($('#frmSolicitud').parsley().isValid({ group: 'informacionPrecalificado' })) {

        //identidad = $("#txtIdentidad").val();
        //ciudad = $("#ddlCiudadResidencia option:selected").val();
        //ingresos = $("#txtIngresos").val().replace(/,/g, '');
        //origenIngresos = $("#ddlOrigenIngreso option:selected").val();
        //telefono = $("#txtTelefono").val();
        //productoSolicitado = $("#ddlProducto option:selected").val();
        //$.ajax({
        //    type: "POST",
        //    url: "SolicitudesCredito_RegistrarAval.aspx/precalificar",
        //    data: JSON.stringify({ identidad: identidad, ciudad: ciudad, ingresos: ingresos, origenIngresos: origenIngresos, telefono: telefono, productoSolicitado: productoSolicitado }),
        //    contentType: 'application/json; charset=utf-8',
        //    error: function (xhr, ajaxOptions, thrownError) {
        //        MensajeError('Error al procesar el precalificado, contacte al administrador');
        //    },
        //    success: function (data) {

        //        if (data.d == true) {
        //            MensajeExito("¡Aval Pre-Aprobado de forma exitosa!");
        //            $(".sw-btn-next").css('display', '');
        //        } else {
        //            MensajeError("El aval no fue aprobado, ingrese otro");
        //        }
        //    }
        //});
        ...
        $('#frmSolicitud').parsley().validate({ group: 'informacionPrecalificado', force: true });
    }
});
...
function llenarDropDownLists() {
    ...
    $.ajax({
        type: "POST",
        url: "SolicitudesCredito_RegistrarAval.aspx/getDDLS",
        contentType: 'application/json; charset=utf-8',
        error: function (xhr, ajaxOptions, thrownError) {
            MensajeError('No se pudo cargar la información, contacte al administrador');
        },
